...
class Attention(nn.Module):
    def __init__(self, d_model):
        ...
        self.proj_1c = nn.Conv2d(d_model//4, d_model//4, 3, padding=1)
        self.activation = nn.GELU()
        self.spatial_gating_unit = LKA(d_model)
        self.proj_2 = nn.Conv2d(d_model, d_model, 1)
        self.proj_2c = nn.Conv2d(d_model//4, d_model//4, 3, padding=1)

    def forward(self, x):
        # 26.6M 5.0G
        # model                        | 25.09M                 | 6.025G
        # model                        | 27.173M                | 6.382G
        # model                        | 28.342M                | 7.184G
        shorcut = x.clone()
        x_ = rearrange(x, 'b (c p1 p2) n1 n2 -> b c (n1 p1) (n2 p2)', p1=2, p2=2)
        x_ = self.proj_1c(x_)
        x_ = rearrange(x_, 'b c (n1 p1) (n2 p2) -> b (c p1 p2) n1 n2', p1=2, p2=2)
...
class OverlapPatchEmbed(nn.Module):
    ...
    def forward(self, x):
        if self.img_size != 224:
            x = rearrange(x, 'b (c p1 p2) n1 n2 -> b c (n1 p1) (n2 p2)', p1=2, p2=2)
        x = self.act(self.norm1(self.proj1(x)))
        x = self.norm2(self.proj2(x))
        x = rearrange(x, 'b c (n1 p1) (n2 p2) -> b (c p1 p2) n1 n2', p1=2, p2=2)
        _, _, H, W = x.shape
        return x, H, W
    ...
    def __init__(self, img_size=224, in_chans=3, num_classes=1000, embed_dims=[64, 128, 256, 512],
        ...
        for i in range(num_stages):
            patch_embed = OverlapPatchEmbed(img_size=img_size if i == 0 else img_size // (2 ** (i + 1)),
                                            ...
                                            in_chans=in_chans if i == 0 else embed_dims[i - 1]//4,
